                ...
                for (const name of workbook.SheetNames) {
                    ...
                    if (sheetType && rawData.length > 0) {
                        ...
                        if (!response.ok) {
                            ...
                            throw new Error(err.error || `Failed to analyze sheet "${name}"`)
                        }
                        ...
                        sheetsMatched++
                        if (result.reasoning) stats.reasoning.push(`[${name}] ${result.reasoning}`)
            ...
            if (selectedType === "everything") {
                ...
                for (const type of importOrder) {
                    ...
                    try {
                        ...
                        const successCount = Number(results?.count || 0)
                        const failedCount = Number(results?.failed || 0)

                        totalSuccess += successCount
                        totalFailed += failedCount
                        byType[type] = { success: successCount, failed: failedCount }

                        if (Array.isArray(results?.errors) && results.errors.length > 0) {
                            allErrors.push(...results.errors.map((e: string) => `[${type}] ${e}`))
                        }
                    } catch (sectionError: any) {
                        const attempted = transformedData.length
                        totalFailed += attempted
                        byType[type] = { success: 0, failed: attempted }
                        allErrors.push(`[${type}] ${sectionError?.message || "Import failed"}`)
                    }
                }
            } else {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-zinc-50 via-white to-emerald-50/30 p-6">
            <div className="max-w-5xl mx-auto">
                <div className="animate-in fade-in slide-up text-center mb-12">
                        ...
                        return (
                            <div key={step.number} className="flex items-center flex-1">
                                <div className="flex flex-col items-center flex-1">
                                    <div
                                        className={`w-12 h-12 rounded-full flex items-center justify-center font-bold transition-all ${isCompleted ? "bg-emerald-500 text-white" : isActive ? "bg-emerald-500 text-white ring-4 ring-emerald-100 animate-in scale-in" : "bg-zinc-200 text-zinc-400"
                                            }`}
                                    >
                ...
                <div
                    key={currentStep}
                    className="animate-in fade-in slide-up bg-white rounded-2xl shadow-xl border border-zinc-200 p-8 md:p-12"
                >
                    {currentStep === 1 && (
                        <div>
                                ...
                                {Object.entries(dataTypeConfig).map(([key, config]) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`p-6 rounded-xl border-2 text-left transition-all hover-scale active-scale ${isSelected ? `border-${config.color}-500 bg-${config.color}-50` : "border-zinc-200 hover:border-zinc-300 bg-white"
                                                }`}
                                        >
                    ...
                    {currentStep === 3 && (
                        <div>
                            ...
                            {previewStats?.reasoning && (
                                <div className="mb-6 bg-zinc-900 rounded-xl p-6 shadow-2xl border border-white/5 relative overflow-hidden group">
                                    ...
                                    <div className="text-zinc-400 font-mono text-xs leading-relaxed max-h-48 overflow-y-auto custom-scrollbar whitespace-pre-wrap">
                                        {previewStats.reasoning}
                                    </div>
                            ...
                            {/* Data Preview Table */}
                            <div className="bg-zinc-50 rounded-xl border border-zinc-200 mb-6 overflow-hidden shadow-sm text-sm">
                            ...
                            <div className="flex justify-between items-center bg-white/80 backdrop-blur-sm p-4 border-t border-zinc-100 -mx-8 -mb-8 mt-4 rounded-b-2xl">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <Button
                                        ...
                                        size="lg"
                                        disabled={isUploading || (previewStats?.errors?.length || 0) > 50 || (previewStats?.count || 0) === 0}
                            ...
                            {importResults.byType && Object.keys(importResults.byType).length > 0 && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-8 max-w-2xl mx-auto text-left">
                                    {Object.entries(importResults.byType).map(([type, result]: [string, any]) => (
                                        <div key={type} className="rounded-lg border border-zinc-200 bg-zinc-50 p-3">
                                            <p className="text-sm font-semibold text-zinc-900 capitalize">{type}</p>
