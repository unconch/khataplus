...
interface HeldCart {
  ...
  requireUpiVerification: boolean
  splitCash: number
  splitUpi: number
  splitCard: number
}

export function SalesFormPos({ inventory, userId, gstInclusive, gstEnabled, showBuyPrice = false, orgId, org }: SalesFormProps) {
  ...
  useEffect(() => {
    const key = `khataplus_held_carts_${orgId}`
    try {
  ...
  const handleBarcodeAdd = () => {
    ...
    const item = inventorySource.find((it) => it.sku.toLowerCase() === code || it.name.toLowerCase() === code)
    if (!item) {
      setError("Barcode/SKU not found")
      return
    }

    const defaultPrice = Number(item.sell_price) > 0 ? Number(item.sell_price) : Number(item.buy_price)
    addToCartWithValues(item, 1, defaultPrice)
    setBarcodeInput("")
  }

  const holdCurrentCart = () => {
    if (cart.length === 0) {
      setError("Add items to cart before holding")
      return
    }
    const name = holdName.trim() || `Hold ${new Date().toLocaleTimeString()}`
    const held: HeldCart = {
      ...
      splitCash: Number.parseFloat(splitCash || "0") || 0,
      splitUpi: Number.parseFloat(splitUpi || "0") || 0,
      splitCard: Number.parseFloat(splitCard || "0") || 0,
    }
    ...
    setRequireUpiVerification(true)
    setSplitCash("")
    setSplitUpi("")
    setSplitCard("")
    setHoldName("")
  ...
  const resumeHeldCart = (id: string) => {
    ...
    setRequireUpiVerification(held.requireUpiVerification)
  ...
  const addToCartWithValues = (item: InventoryItem, qty: number, price: number) => {
    if (qty > item.stock) {
  ...
  const addToCart = () => {
    ...
    addToCartWithValues(selectedItem, qty, price)
  ...
  const splitBalance = grandTotals.totalAmount - splitTotal
  ...
  const canUseSplit = paymentMethod === "Split"
  ...
  const proceedToReview = () => {
    if (canUseSplit && !splitBalanced) {
      setError("Split payment total must match grand total before review.")
      return
    }
    setError(null)
    setStep(4)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === "Split" && !splitBalanced) {
      setError("Split payment is not balanced with grand total.")
      setIsLoading(false)
      return
    }
        ...
        <form onSubmit={handleSubmit} className="px-4 md:px-7 pt-4 md:pt-6 pb-4 md:pb-7 space-y-5">
          {step === 1 && (
            <div className="space-y-5">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      ...
                      if (e.key === "ArrowDown") {
                        ...
                        e.preventDefault()
                        setActiveSuggestionIndex((prev) => (prev <= 0 ? filteredItems.length - 1 : prev - 1))
                      } else if (e.key === "Enter") {
                        e.preventDefault()
                        const idx = activeSuggestionIndex >= 0 ? activeSuggestionIndex : 0
                        handleSelectItem(filteredItems[idx])
                      }
                    }}
                  />
                </div>
                  ...
                  <div className="absolute z-[120] left-2 right-2 top-full mt-2 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-2xl overflow-hidden max-h-[min(36dvh,320px)] overflow-y-auto">
                    {filteredItems.map((item, idx) => (
                      <button
                        ...
                        key={item.id}
                        className={cn(
                          "w-full px-4 py-3.5 text-left border-b border-zinc-100 dark:border-zinc-800 last:border-0 transition-colors flex items-center justify-between gap-3",
              ...
              <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/70 dark:bg-zinc-900/40 p-3 md:p-4 space-y-3">
                <Label className="text-[11px] font-black uppercase tracking-[0.16em] text-zinc-500">POS Shortcuts</Label>
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                  <div className="md:col-span-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 h-11 px-3 flex items-center gap-2">
                    <ScanLineIcon className="h-4 w-4 text-zinc-500" />
                    <input
                      className="w-full bg-transparent outline-none text-sm font-medium"
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="md:col-span-2 h-11 rounded-xl font-semibold"
                  ...
                  <div className="md:col-span-4 flex items-center gap-2">
                    <input
                      className="h-11 w-full rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-3 text-sm"
                      placeholder="Hold cart name (optional)"
                      value={holdName}
                      onChange={(e) => setHoldName(e.target.value)}
                    />
                    <Button type="button" variant="outline" className="h-11 rounded-xl shrink-0" onClick={holdCurrentCart} disabled={cart.length === 0}>
                ...
                {heldCarts.length > 0 && (
                  <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-2.5 space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-zinc-500 px-1">Held Carts</p>
                    <div className="space-y-1.5 max-h-40 overflow-y-auto">
                      {heldCarts.map((held) => (
                        <div key={held.id} className="flex items-center justify-between gap-2 rounded-lg border border-zinc-100 dark:border-zinc-800 px-2.5 py-2">
                          <div className="min-w-0">
                            <p className="text-xs font-semibold truncate">{held.name}</p>
                            <p className="text-[11px] text-muted-foreground">{held.cart.length} items â€¢ {new Date(held.createdAt).toLocaleTimeString()}</p>
                          </div>
                          <Button type="button" size="sm" variant="outline" className="h-8 shrink-0" onClick={() => resumeHeldCart(held.id)}>
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              {paymentMethod === "Split" && (
                <div className="rounded-2xl border border-blue-200 dark:border-blue-900/50 bg-blue-50/60 dark:bg-blue-950/20 p-4 space-y-3">
                  <p className="text-xs font-semibold text-blue-800 dark:text-blue-300">
                    Split payment: enter received amount by mode.
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <input
                      type="number"
                      step="0.01"
                      className="h-11 w-full rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-3 text-sm"
                      placeholder="Cash"
                      value={splitCash}
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold">Rs{splitTotal.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-zinc-600">Balance</span>
                    <span className={cn("font-semibold", splitBalanced ? "text-emerald-700 dark:text-emerald-300" : "text-rose-600")}>
                      Rs{splitBalance.toFixed(2)}
                    </span>
                  </div>
                </div>
              )}
              ...
              <div className="sticky bottom-0 pt-3 flex items-center justify-between bg-white/95 dark:bg-zinc-950/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
                ...
                <Button type="button" onClick={proceedToReview} className="rounded-xl font-black uppercase tracking-wide">
                  Continue
                  <ArrowRight className="h-4 w-4 ml-2" />
                </Button>
              </div>
                ...
                {paymentMethod === "Split" && (
                  <>
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">Rs{splitCashNum.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">UPI</span>
                      <span className="font-medium">Rs{splitUpiNum.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Card</span>
                      <span className="font-medium">Rs{splitCardNum.toFixed(2)}</span>
                    </div>
                  </>
                )}
