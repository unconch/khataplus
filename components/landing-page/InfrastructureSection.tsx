...
export function InfrastructureSection({ whiteTheme = false }: { whiteTheme?: boolean }) {
    return (
        <section id="infrastructure" className={cn("py-24 md:py-40 relative overflow-hidden", whiteTheme ? "bg-white text-zinc-900" : "bg-[#09090b] text-white")}>
            {/* Background elements */}
                    ...
                    <div className="flex-1 w-full order-2 lg:order-1 relative z-10">
                        <AdvancedScrollReveal variant="scaleUp">
                            <div className="relative p-1 bg-gradient-to-tr from-white/10 to-transparent rounded-[3.5rem]">
                                <div className="relative bg-zinc-950 backdrop-blur-3xl rounded-[3.2rem] p-12 overflow-hidden shadow-2xl border border-white/5">
                                    ...
                                    <div className="relative z-10 space-y-12">
                                        ...
                                        <div className="p-8 rounded-3xl bg-zinc-900 text-white relative overflow-hidden group">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-zinc-400 text-xs">Continuous monitoring and automated
                    ...
                    <div className="flex-1 space-y-12 order-1 lg:order-2 text-center lg:text-left relative z-10">
                        <AdvancedScrollReveal variant="slideLeft">
                            <div className="space-y-6">
                                <div className={cn("inline-flex items-center gap-2 px-3 py-1.5 rounded-full border shadow-sm", whiteTheme ? "bg-zinc-50 border-zinc-200" : "bg-white/5 border-white/10 backdrop-blur-md")}>
                                    <Server size={14} className={whiteTheme ? "text-zinc-600" : "text-white"} />
                                    <span className={cn("font-bold text-[10px] tracking-widest uppercase", whiteTheme ? "text-zinc-600" : "text-white")}>Infrastructure</span>
                                </div>
                                <h2 className={cn("text-5xl md:text-7xl font-bold tracking-tightest leading-[1.1]", whiteTheme ? "text-zinc-900" : "text-white")}>
                                    Engineered for <br />
                                    <GradientText className="inline" colors={["#60a5fa", "#a855f7", "#fb923c"]}>
                                        Total Reliability.
                                    </GradientText>
                                </h2>
                                <p className={cn("text-lg font-normal leading-relaxed max-w-lg mx-auto lg:mx-0", whiteTheme ? "text-zinc-500" : "text-zinc-400")}>
                            ...
                            {[
                                ...
                                <AdvancedScrollReveal key={i} variant="slideUp" delay={200 + (i * 100)}>
                                    <div className="flex flex-col md:flex-row items-center lg:items-start gap-5">
                                        <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center border flex-shrink-0", whiteTheme ? "bg-zinc-50 border-zinc-200 text-zinc-600" : "bg-white/5 border-white/10 text-white backdrop-blur-md")}>
                                            <Shield size={20} strokeWidth={1.5} />
                                        </div>
                                        <div className="space-y-1">
                                            <h4 className={cn("text-xl font-bold tracking-tight leading-none", whiteTheme ? "text-zinc-900" : "text-white")}>{item.title}</h4>
                                            <p className={cn("text-sm font-normal", whiteTheme ? "text-zinc-500" : "text-zinc-400")}>{item.desc}</p>
                                        </div>
                                    </div>
                                </AdvancedScrollReveal>
                            ))}
