...
import { getExecutiveAnalytics, getProfile } from "@/lib/data"
...
export default async function ExecutiveDashboard() {
    ...
    if (!currSession) redirect("/auth/login")

    const userId = (currSession as any)?.token?.sub || (currSession as any)?.user?.userId
    const profile = await getProfile(userId)

    // Only Owner and Main Admin can see this
    if (profile?.role !== "owner" && profile?.role !== "main admin") {
        redirect("/dashboard")
    }
