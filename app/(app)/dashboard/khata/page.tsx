  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="border-none bg-emerald-50 shadow-sm dark:bg-emerald-500/10">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 mb-1">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider">{copy.khata.youWillGet}</span>
            </div>
            <div className="flex items-center text-xl font-bold text-emerald-700 dark:text-emerald-400">
              <IndianRupee className="h-4 w-4" />
              {formatNumber(totalGet, locale)}
            </div>
          </CardContent>
        </Card>
        <Card className="border-none bg-rose-50 shadow-sm dark:bg-rose-500/10">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-rose-600 dark:text-rose-400 mb-1">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider">{copy.khata.youWillGive}</span>
            </div>
            <div className="flex items-center text-xl font-bold text-rose-700 dark:text-rose-400">
              <IndianRupee className="h-4 w-4" />
              {formatNumber(totalGive, locale)}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Link href="/dashboard/customers" className="block">
          <Card className="hover:bg-muted/50 transition-colors border-none shadow-sm bg-muted/20">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground font-medium uppercase tracking-tight">{copy.khata.totalParties}</p>
                <p className="text-lg font-bold">{formatNumber(customers.length, locale)}</p>
              </div>
            </CardContent>
          </Card>
        </Link>
        <Card className="border-none shadow-sm bg-muted/20">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground font-medium uppercase tracking-tight">{copy.khata.entries}</p>
              <p className="text-lg font-bold">{formatNumber(transactions.length, locale)}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold px-1">{copy.khata.recentTransactions}</h3>
        <div className="space-y-3">
          {transactions.length === 0 ? (
            <div className="text-center py-12 bg-muted/20 rounded-xl border border-dashed">
              <p className="text-muted-foreground">{copy.khata.noRecentEntries}</p>
            </div>
          ) : (
            transactions.map((tx) => {
              const resolvedCustomer = customerById.get(tx.customer_id)
              const customerName = resolvedCustomer?.name || tx.customer?.name || copy.khata.unknownCustomer
              return (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground font-medium">
                          {formatDate(tx.created_at, locale, { day: "numeric", month: "short" })} -
                          {tx.type === "credit" ? ` ${copy.khata.gave}` : ` ${copy.khata.got}`}
                        </p>
                      </div>
                    </div>
                    <div className={cn(
                      ...
                      {formatNumber(Number(tx.amount), locale)}
                    </div>
                  </div>
                </Link>
              )
