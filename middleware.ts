...
const SYSTEM_PREFIXES = new Set([
    ...
    'dashboard', 'demo', 'marketing', 'offline', 'docs', 'solutions', 'pos',
    'pending-approval', 'tools', 'beta', 'for', 'shop', 'features',
])
...
export default async function middleware(req: NextRequest) {
    ...
    // If first segment is NOT a system route, treat it as an org slug (app surface)
    if (!isPosHost && firstSegment && !SYSTEM_PREFIXES.has(firstSegment) && !firstSegment.includes('.')) {
    ...
    // --------------------------------------------------------------------------
    // ORG ROUTE GUARD: Require session and active org metadata for org routes.
    // --------------------------------------------------------------------------
    if (orgSlug) {
        if (!user) {
            const nextParam = encodeURIComponent(pathname + url.search)
            return NextResponse.redirect(new URL(`/auth/login?next=${nextParam}`, req.url))
        }

        const activeSlug = String(user.user_metadata?.active_org_slug || "").trim()
        if (!activeSlug) {
